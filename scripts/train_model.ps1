param(
    ...
    [int]$StatefulBoundaryTokens = 64,
    [double]$StatefulBoundaryDecay = 0.97,
    [double]$LambdaStatefulBoundary = 0.0,
    [double]$LambdaStatefulFull = 0.0,
    [double]$StatefulTargetMargin = 0.0,
...
if ($isStateModel) {
    ...
    if ($Model -in @("naime_v6_recursive_self_moe", "naime_v7_typed_dynamics")) {
        $common += @(
            ...
            "--stateful-boundary-tokens", "$StatefulBoundaryTokens",
            "--stateful-boundary-decay", "$StatefulBoundaryDecay",
            "--lambda-stateful-boundary", "$LambdaStatefulBoundary",
            "--lambda-stateful-full", "$LambdaStatefulFull",
            "--stateful-target-margin", "$StatefulTargetMargin",
