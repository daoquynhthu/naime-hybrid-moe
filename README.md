...
V6 (`naime_v6_recursive_self_moe`) is the active architecture family, but only clean causal-integrity-v2 runs should be used for current claims. Older continuation runs that produced ultra-low perplexity are treated as legacy/contaminated evidence and must not be used as proof of architecture quality.
